...
def gaussian1Derf(p, x, y):
    return lambda p, x, y: gaussian1D(x,*p) - y
...
def fit1DGaussian(data, sigma):
    ...
    p0 = [numpy.min(profile), numpy.max(profile), numpy.argmax(profile), sigma]
    #print(p0)
    result = p0
    errorfunction = lambda p, x, y: gaussian1D(x,*p) - y
    #fit full profile
    [result, cov_x, infodict, mesg, success] = scipy.optimize.leastsq(errorfunction, p0, args=(x_ind,profile), full_output = 1, maxfev = 500)
...
# Testing
# AH testing single spot fitting here
if (__name__ == "__main__"):
    ...
    if False:
        data = plt.imread('testframe.tif')
        print(fit1DGaussian(data, 10))
